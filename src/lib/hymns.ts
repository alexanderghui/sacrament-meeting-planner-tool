// Bundled hymn reference — number → title. Two collections:
//  • 1985 "Hymns of The Church of Jesus Christ of Latter-day Saints" (1–341)
//  • "Hymns—for Home and Church" (1001+, released in batches; non-sequential)
// Bundled (not fetched) so the picker is instant and works offline. Titles
// overlapping the ward's own data were cross-checked against the DB.

export const HYMN_TITLES: Record<number, string> = {
...
export type Hymn = { number: number; title: string };

export const HYMNS: Hymn[] = Object.entries(HYMN_TITLES).map(([n, title]) => ({
  number: Number(n),
  title,
}));

export function hymnTitle(n: number | null | undefined): string | null {
  return n != null ? HYMN_TITLES[n] ?? null : null;
}
